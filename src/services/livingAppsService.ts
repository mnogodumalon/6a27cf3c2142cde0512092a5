...
import type { Zutaten, ApfelstrudelVarianten, CreateZutaten, CreateApfelstrudelVarianten } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async createZutatenEntry(fields: CreateZutaten) {
    return callApi('POST', `/apps/${APP_IDS.ZUTATEN}/records`, { fields: cleanFieldsForApi(fields as any, 'zutaten') });
  }
  static async updateZutatenEntry(id: string, fields: Partial<CreateZutaten>) {
    return callApi('PATCH', `/apps/${APP_IDS.ZUTATEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'zutaten') });
  }
  ...
  static async getApfelstrudelVarianten(): Promise<ApfelstrudelVarianten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.APFELSTRUDEL_VARIANTEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as ApfelstrudelVarianten[];
    return enrichLookupFields(records, 'apfelstrudel_varianten');
  }
  static async getApfelstrudelVariantenEntry(id: string): Promise<ApfelstrudelVarianten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.APFELSTRUDEL_VARIANTEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as ApfelstrudelVarianten;
    return enrichLookupFields([record], 'apfelstrudel_varianten')[0];
  }
  static async createApfelstrudelVariantenEntry(fields: CreateApfelstrudelVarianten) {
    return callApi('POST', `/apps/${APP_IDS.APFELSTRUDEL_VARIANTEN}/records`, { fields: cleanFieldsForApi(fields as any, 'apfelstrudel_varianten') });
  }
  static async updateApfelstrudelVariantenEntry(id: string, fields: Partial<CreateApfelstrudelVarianten>) {
    return callApi('PATCH', `/apps/${APP_IDS.APFELSTRUDEL_VARIANTEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'apfelstrudel_varianten') });
  }
  static async deleteApfelstrudelVariantenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.APFELSTRUDEL_VARIANTEN}/records/${id}`);
  }
